...
class PropDataModule(pl.LightningDataModule):
    def __init__(self, x, y, batch_size, gen_mols):
        ...
        # self.train_data, self.test_data = random_split(self.dataset, [500, len(self.dataset) - 500])
        # self.train_data, self.test_data = Subset(self.dataset, range(gen_mols)), Subset(self.dataset, range(gen_mols, len(self.dataset)))
        self.train_data, self.test_data = random_split(Subset(self.dataset, range(gen_mols, len(self.dataset))),
                                                [int(round((len(self.dataset) - gen_mols) * 0.7)), int(round((len(self.dataset) - gen_mols) * 0.3))])
        self.train_data = ConcatDataset([self.train_data, Subset(self.dataset, range(gen_mols))])
...
def smile_to_one_hot(smile):
    out = torch.zeros((dm.dataset.max_len, len(dm.dataset.symbol_to_idx)))
    for index in smiles_to_indices(smile):
        out[0][index] = 1
    return out.flatten()
...
def smile_to_vec(smile, vae):
    return vae.encode(smiles_to_indices(smile).unsqueeze(0).to(device))

def smile_to_z(smile, vae):
    zs = torch.zeros((1, 1024), device=device)
    target = smile_to_one_hot(smile).to(device)
    z = vae.encode(smiles_to_indices(smile).unsqueeze(0).to(device))[0].detach().requires_grad_(True)
    optimizer = torch.optim.Adam([z], lr=0.1)
    for epoch in range(10000):
        optimizer.zero_grad()
...
def smiles_to_affinity(smiles, autodock, protein_file, num_devices=torch.cuda.device_count()):
    ...
    for i, hot in enumerate(tqdm(smiles, desc='preparing ligands')):
        with open('ligand_smiles.txt', 'a') as file:
                file.write(f'{i}, {hot}\n')
        if "." in hot:
            with open('error_smiles.txt', 'a') as file:
                file.write(f'{i}, {hot}\n')
            continue
    ...
    if len(smiles) == 1:
        ...
        ps = []
        for device in range(num_devices):
            ps.append(subprocess.Popen(f'{autodock} -M {protein_file} -s 0 -B ligands/{device}/ligand*.pdbqt -N ../../outs/ -D {device + 1}', shell=True))
        stop = False
        while not stop: 
            for p in ps:
                stop = True
